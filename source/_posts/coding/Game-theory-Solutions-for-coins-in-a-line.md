...
For `not (dp[i-1] and dp[i-2])`, `dp[i]` is `True` when and only when there is a `False` in `dp[i]`'s sub states
`dp[i-1]` and `dp[i-2]`. Which means, there is a possible lose for his opponent. `dp[i]`'s sub states represent the
